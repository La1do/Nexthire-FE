...
export function AdminActionMenu({ items, label }: Props) {
  ...
  return <div className="admin-action-menu" ref={menuRef}>
    <button aria-controls={menuId} aria-expanded={isOpen} aria-haspopup="menu" aria-label={label} className="admin-action-menu__trigger" onClick={() => setIsOpen((current) => !current)} title={label} type="button"><span className="sr-only">{label}</span></button>
    {isOpen ? <div className="admin-action-menu__popover" id={menuId} role="menu">
