...
type Props = {
  content: AdminSettingsTranslations['password']
  isPending: boolean
  onSave: (payload: ChangePasswordPayload) => Promise<unknown>
}

function apiError(error: unknown, content: AdminSettingsTranslations['password']) {
  const envelope = getApiErrorEnvelope(error)

  if (envelope?.error.code === 'AUTH.INVALID_CREDENTIALS') {
    return content.apiErrors.invalidCredentials
  }

  if (envelope?.error.code === 'AUTH.PASSWORD_REUSE_NOT_ALLOWED') {
    return content.apiErrors.passwordReuse
  }

  if (envelope?.error.code === 'AUTH.USER_CREDENTIAL_NOT_FOUND') {
    return content.apiErrors.credentialMissing
  }
...
export function AdminChangePasswordForm({ content, isPending, onSave }: Props) {
  ...
  const schema = useMemo(
    () => z.object({
      currentPassword: z
        .string()
        .min(1, content.validation.currentRequired)
        .min(8, content.validation.length)
        .max(128, content.validation.length),
      newPassword: z
        .string()
        .min(1, content.validation.nextRequired)
        .min(8, content.validation.length)
        .max(128, content.validation.length),
      ...
      if (values.currentPassword && values.currentPassword === values.newPassword) {
        context.addIssue({
          code: 'custom',
          message: content.validation.reuse,
          path: ['newPassword'],
        })
      }
  ...
  const submit = handleSubmit(async (values) => {
    try {
      ...
      toast.error(apiError(error, content))
    }
  })
  ...
  return (
    <section className="admin-settings-card">
      ...
      <form className="admin-settings-password-form" noValidate onSubmit={submit}>
        ...
        <div className="admin-settings-password-rules">
          <strong>{content.rulesTitle}</strong>
          <ul>
            {rules.map(([valid, label]) => (
              <li className={valid ? 'is-valid' : ''} key={label}>
